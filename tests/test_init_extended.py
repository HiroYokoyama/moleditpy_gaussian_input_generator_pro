...
Extended coverage for gaussian_input_generator_pro/__init__.py, focused on
the run() function and the on_reset() "dialog still open" deferral path,
neither of which tests/test_plugin_integration.py exercises (that file only
tests initialize()'s registration contract and the save/load/reset handlers
in isolation, via a StubPluginContext, without ever calling run()).

run() constructs a real GaussianSetupDialogPro, so this file loads a
private, real-PyQt6/RDKit-bound copy of the whole plugin module tree -- see
...
shared "gaussian_input_generator_pro.*" sys.modules cache slots, independent
of collection order).
"""
...
_PRIV_PKG = "_gigp_real_qt_copy"  # shared with test_main_dialog_extended.py
...
class TestRunFilenameDetection(_BaseInitTestCase):
    def _run_and_capture_filename(self, mw):
        ...
        with patch.object(main_dialog_mod, "GaussianSetupDialogPro", _FakeDialog):
...
class TestRunExistingDialog(_BaseInitTestCase):
    def test_visible_existing_dialog_is_raised_not_recreated(self):
        ...
        with patch.object(main_dialog_mod, "GaussianSetupDialogPro") as mock_cls:
            _init_mod.run(None)
...
class TestRunFullFlow(_BaseInitTestCase):
    def test_creates_and_registers_real_dialog(self):
        ...
        self.assertIsInstance(dlg, main_dialog_mod.GaussianSetupDialogPro)
...
# ---------------------------------------------------------------------------
# initialize()'s show_dialog closure
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# on_save() / on_load() -- persistence gating and key filtering
# ---------------------------------------------------------------------------


class TestSaveLoadHandlers(_BaseInitTestCase):
    def setUp(self):
        super().setUp()
        self.ctx = StubPluginContext()
        _init_mod.initialize(self.ctx)
        self.on_save = self.ctx._save_handlers[0]
        self.on_load = self.ctx._load_handlers[0]

    def test_on_save_returns_none_before_dialog_opened(self):
        self.assertIsNone(self.on_save())

    def test_on_save_returns_settings_after_dialog_opened(self):
        _init_mod._dialog_opened = True
        result = self.on_save()
        self.assertIs(result, _init_mod.current_settings)

    def test_on_load_filters_molecule_specific_keys(self):
        self.on_load({"nproc": 8, "charge": 1, "mult": 2})
    ...
    def test_on_load_empty_dict_ignored(self):
        _init_mod._dialog_opened = False
        self.on_load({})
        self.assertFalse(_init_mod._dialog_opened)

    def test_on_load_non_dict_ignored(self):
        _init_mod._dialog_opened = False
        self.on_load(None)
        self.assertFalse(_init_mod._dialog_opened)
