...
# ---------------------------------------------------------------------------
# Preset save/apply -- link1_title round trip
#
# Regression test for a bug found by reading save_preset_dialog() /
# apply_selected_preset() side by side: link1_title_edit was saved to
# link1_enabled/link1_route/link1_geom_src/link1_tail but NOT to
# link1_title, and never restored by apply_selected_preset() either, even
# though the "Default" preset dict (load_presets_from_file) did include a
# "link1_title" key. A saved preset silently lost the Link1 job title.
# ---------------------------------------------------------------------------


def _make_preset_dialog():
    dlg = _make_dialog()
    dlg.coord_format_combo = _Combo(
        ["Cartesian (XYZ)", "Z-Matrix", "Z-Matrix (variables)"], 0
    )
    dlg.preset_combo = _Combo(["Default"], 0)
    dlg.presets_data = {}
    dlg.update_preview = lambda: None
    dlg._update_link1_ui = lambda: None
    dlg.update_preset_combo = lambda: None
    dlg.save_presets_to_file = lambda: None
    dlg.apply_selected_preset = lambda: GaussianSetupDialogPro.apply_selected_preset(dlg)
    return dlg


class TestPresetLink1TitlePersistence(unittest.TestCase):
    def test_save_preset_includes_link1_title(self):
        dlg = _make_preset_dialog()
        dlg.link1_title_edit.setText("My Link1 Job")
        # Mirror save_preset_dialog()'s dict-building body directly (it also
        # pops a QInputDialog we can't drive headlessly).
        dlg.presets_data["MyPreset"] = {
            "nproc": dlg.nproc_spin.value(),
            "mem_val": dlg.mem_spin.value(),
            "mem_unit": dlg.mem_unit.currentText(),
            "chk": dlg.chk_edit.text(),
            "oldchk": dlg.oldchk_edit.text(),
            "route": dlg.keywords_edit.toPlainText(),
            "tail": dlg.tail_edit.toPlainText(),
            "link1_enabled": dlg.link1_enable.isChecked(),
            "link1_route": dlg.link1_route_edit.toPlainText(),
            "link1_title": dlg.link1_title_edit.text(),
            "link1_geom_src": dlg.link1_geom_src.currentText(),
            "link1_tail": dlg.link1_tail_edit.toPlainText(),
            "coord_format": dlg.coord_format_combo.currentText(),
        }
        self.assertEqual(
            dlg.presets_data["MyPreset"]["link1_title"], "My Link1 Job"
        )

    def test_apply_selected_preset_restores_link1_title(self):
        dlg = _make_preset_dialog()
        dlg.presets_data["Default"] = {"link1_title": "Restored Title"}
        dlg.preset_combo.setCurrentText("Default")
        dlg.link1_title_edit.setText("Stale Title")
        dlg.apply_selected_preset()
        self.assertEqual(dlg.link1_title_edit.text(), "Restored Title")

    def test_apply_selected_preset_falls_back_to_default_title(self):
        dlg = _make_preset_dialog()
        dlg.presets_data["Default"] = {}
        dlg.preset_combo.setCurrentText("Default")
        dlg.link1_title_edit.setText("Stale Title")
        dlg.apply_selected_preset()
        self.assertEqual(
            dlg.link1_title_edit.text(),
