...
class TestCloseAndDocumentState(_RealDialogTestCase):
    ...
    def test_reject_asks_before_discarding_unsaved_changes(self):
        ...
        dlg = self._make_dialog(mol=_make_water())
        dlg.current_inp_file = os.path.join(self._tmpdir, "esc.gjf")
        dlg._saved_inp_content = "OLD CONTENT"
        with patch.object(
            main_dialog_mod.QMessageBox,
            "question",
    ...
    def test_reject_discards_when_the_user_says_so(self):
        dlg = self._make_dialog(mol=_make_water())
        dlg.current_inp_file = os.path.join(self._tmpdir, "esc2.gjf")
        dlg._saved_inp_content = "OLD CONTENT"
        with patch.object(
            main_dialog_mod.QMessageBox,
            "question",
    ...
    def test_close_event_cancel_leaves_the_builder_open(self):
        ...
        dlg = self._make_dialog(mol=_make_water())
        dlg.current_inp_file = os.path.join(self._tmpdir, "keep.gjf")
        dlg._saved_inp_content = "OLD CONTENT"
        dlg.builder_dialog = MagicMock()
        event = QCloseEvent()
        with patch.object(
...
class TestSubmitToClusterButton(_RealDialogTestCase):
    ...
    def test_clicking_saves_first_then_hands_off(self):
        ...
        dlg = self._make_dialog(parent=host, mol=_make_water())
        target = os.path.join(self._tmpdir, "run.gjf")
    ...
    def test_an_already_saved_clean_file_is_not_saved_again(self):
        ...
        host = self._host([{"name": "Job Manager", "module": module}])
        dlg = self._make_dialog(parent=host, mol=_make_water())
        target = os.path.join(self._tmpdir, "clean.gjf")
        with open(target, "w", encoding="utf-8") as handle:
            handle.write("#p HF")
        dlg.current_inp_file = target
        dlg._saved_inp_content = dlg.preview_text.toPlainText()
    ...
    def test_a_refusal_is_reported_to_the_user(self):
        ...
        host = self._host([{"name": "Job Manager", "module": module}])
        dlg = self._make_dialog(parent=host, mol=_make_water())
        target = os.path.join(self._tmpdir, "refused.gjf")
        with open(target, "w", encoding="utf-8") as handle:
            handle.write("#p HF")
        dlg.current_inp_file = target
        dlg._saved_inp_content = dlg.preview_text.toPlainText()
