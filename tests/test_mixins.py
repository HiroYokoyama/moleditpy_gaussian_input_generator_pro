...
GaussianRouteBuilderDialog (and, via the same mixin, other MoleditPy plugin
...
# ...) fake out gaussian_input_generator_pro.mixins with a stub
# Dialog3DPickingMixin and/or clobber sys.modules["PyQt6*"] with permissive
# stub modules, with no regard for import order -- whichever test file is
# collected first "wins" the shared cache slots. Load a fully private copy of
# mixins.py against genuine PyQt6 (recovered from the "__real" stash
# conftest.py sets aside) so this file's coverage always exercises the
# genuine module regardless of what other test files have done to the
# canonical sys.modules entries.
# ---------------------------------------------------------------------------
...
def _load_private_mixins():
    ...
    path = os.path.join(_REPO_ROOT, "gaussian_input_generator_pro", "mixins.py")
